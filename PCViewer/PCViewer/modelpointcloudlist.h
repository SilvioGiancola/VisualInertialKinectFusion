...
public slots:
    ...
    bool movePointCloud(PointCloudT::Ptr PC, Eigen::Matrix4f Mat);
private:
...
// TODO : calculate inverse and apply inverse
class  Move_Command : public QUndoCommand
{
public:
    Move_Command(ModelPointCloudList *m, PointCloudT::Ptr myold, Eigen::Matrix4f Transf)
    {

        _m = m;
        _m->movePointCloud(myold, Transf);
        _old = myold;
        _trans = Transf;
     ...
     /*   _old.reset(new PointCloudT);
        ...
        setText(QString("[Transform] %1").arg(QString::fromStdString(myold->header.frame_id).section("/",-1,-1)));
    }
    ...
    //    inverse.
        _m->movePointCloud(_old, inverse);
        //_m->(_new, _old);
       // _m->replacePointCloudToModel(_new, _old);
    }
    void redo()
    {
        _m->movePointCloud(_old, _trans);
       // _m->replacePointCloudToViewer(_old, _new);
      //  _m->replacePointCloudToModel(_old, _new);
    }
